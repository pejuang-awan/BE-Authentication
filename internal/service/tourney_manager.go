...
type (
	TourneyManager interface {
		...
		GetTourneyAndTeamCount(gameId string) ([]byte, int, error)
		UpdateTeamCount(reqBytes []byte) ([]byte, int, error)
	}
...
func (t *tourneyManagerService) GetTourneyAndTeamCount(gameID string) ([]byte, int, error) {
	url := t.deps.Config.Services.TourneyManagerURL + getCountURL + "/" + gameID
...
func (t *tourneyManagerService) UpdateTeamCount(reqBytes []byte) ([]byte, int, error) {
	url := t.deps.Config.Services.TourneyManagerURL + updateTeamCountURL

	t.deps.Logger.Infof("Calling tourney manager service url %s", url)

	response, err := httpCall(http.MethodPost, url, bytes.NewReader(reqBytes))
	if err != nil || response.StatusCode != http.StatusOK {
		t.deps.Logger.Errorf("Error when send request to update team count in tourney manager service: %v", err)
		return nil, response.StatusCode, err
	}

	t.deps.Logger.Info("Success when send request to update team count in tourney manager service")
